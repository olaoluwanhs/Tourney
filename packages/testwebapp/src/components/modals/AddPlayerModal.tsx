...
import type { Draws, Player, PlayerId, PlayerUser } from "../../hooks/useTournament";
...
type AddPlayerModalProps =
    ...
    | {
        ...
        /** Already-assigned position-reference values, e.g. ["1-t1m1"] */
        assignedRefs: string[];
        onClose: () => void;
...
export function AddPlayerModal(props: AddPlayerModalProps) {
    ...
    const [tab, setTab] = useState<"roster" | "reference">("roster");
    ...
    // ── Assign mode ──────────────────────────────────────────────
    const { gameId, previousDraws, roster, assignedPlayerIds, assignedRefs, onAssign, onClose } = props;
    ...
    const selectedMatchMeta = previousMatches.find((m) => m.matchId === selectedMatchId);
                        ...
                        <div className="player-list">
                            {userPlayers.map((p) => {
                                ...
                                return (
                                    ...
                                    >
                                        {p.associatedImage ? (
                                            <img src={p.associatedImage} alt={p.name} className="player-avatar" />
                                        ) : (
            ...
            {activeTab === "reference" && (
                <>
                    <p className="apm-ref-hint">
                        ...
                        in the selected match. Format: <code>&lt;position&gt;-&lt;matchId&gt;</code>
                    </p>
